...
class Command:
    ...
    async def handle_command(self, client, message, input_list):
        try:
            ...
            logger.bind(input_list=input_list).exception("Unknown error when handling Discord command")

            mention_str = client.get_mention_str(commands_util.ADMIN_DISC_ID, message.guild.id)
            response = (
