...
def home(error_msg=None, status=200):
    ...
    for list_id, owner_id, list_name, list_count in lists:
        user_data = app_util.discord_request("func", ["get_discord_nick", "get_discord_avatar"],  owner_id)
        avatar = flask.url_for("static", _external=True, filename=app_util.get_relative_static_folder(user_data[1], config))
    ...
    return app_util.make_template_context("lists.html", status, lists=list_data, error=error_msg)
    ...
    list_items = [
        (
            ...
            flask.url_for(
                "static",
                _external=True,
                filename=app_util.get_relative_static_folder(riot_api.get_champ_portrait_path(champ_id), config)
            )
...
def rename(list_id):
    ...
    if logged_in_user is None: # Not logged in.
        ...
        return app_util.make_text_response(error_msg, 401)

    success, response = lists.rename_list(logged_in_user, list_id, data.get("name"), database)

    if not success:
        return app_util.make_text_response(f"List could not be renamed: {response}.", 400)

    return app_util.make_text_response("success", 200)
...
def delete_item(item_id):
    ...
    if not success:
        return app_util.make_text_response(f"Can't delete champion: {response}.", 400)
