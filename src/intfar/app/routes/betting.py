...
from intfar.api.betting import MAX_BETTING_THRESHOLD
...
def get_bets(database, bet_handler, only_active):
    ...
    avatars = {
        disc_id: flask.url_for("static", _external=True, filename=app_util.get_relative_static_folder(avatars[disc_id], database.config))
...
def home():
    ...
    for balance, disc_id in all_balances:
        ...
        avatar = flask.url_for("static", _external=True, filename=app_util.get_relative_static_folder(all_avatars[disc_id], config))
