...
from intfar.api.util import JEOPARDY_ITERATION, JEOPADY_EDITION, JEOPARDY_REGULAR_ROUNDS, MONTH_NAMES
from intfar.api.lan import is_lan_ongoing, get_latest_lan_info
...
QUESTIONS_FILE = f"intfar/app/static/data/jeopardy_questions_{JEOPARDY_ITERATION}.json"
USED_QUESTIONS_FILE = f"intfar/app/static/data/jeopardy_used_{JEOPARDY_ITERATION}.json"
...
def get_round_data(request_args, config):
    player_data = []
...
def question_view(jeopardy_round, category, tier):
    ...
    with flask.current_app.config["JEOPARDY_JOIN_LOCK"]:
        ...
        player_data, player_turn, question_num = get_round_data(flask.request.args, config)
...
def final_jeopardy():
    ...
    player_data = get_round_data(flask.request.args, config)[0]
