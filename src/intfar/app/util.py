...
def get_relative_static_folder(filename: str, config: Config):
    filename_parts = filename.replace("\\", "/")

    return filename_parts.replace(f"{config.static_folder}/", "")
...
def get_user_details():
    ...
    if logged_in_user is not None:
        ...
        if avatar is not None:
            logged_in_avatar = flask.url_for("static", _external=True, filename=get_relative_static_folder(avatar, config))
