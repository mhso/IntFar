...
from intfar.api.game_stats import GameStats, PlayerStats, GameStatsParser
from intfar.api.util import format_duration
...
class CS2PlayerStats(PlayerStats):
    ...
    def get_formatted_stat_value(cls, stat, value) -> dict[str, str]:
        ...
        if stat in ("headshot_pct", "accuracy"):
            return fmt_value + "%"
...
class CS2GameStatsParser(GameStatsParser[SteamAPIClient]):
    ...
    def parse_from_database(self, database, game_id: int = None) -> list[GameStats]:
        ...
        map_names = self.api_client.map_names
        all_stats = []
        for game_stats, player_stats in zip(all_game_stats, all_player_stats):
            game_stats["map_name"] = map_names.get(game_stats["map_id"])
