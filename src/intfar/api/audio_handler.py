...
class AudioHandler:
    ...
    async def _download_from_url(self, url: str):
        with open(f"{self.config.resources_folder}/user_agent.txt", "r", encoding="utf-8") as fp:
        ...
        args = [
            ...
            user_agent,
            "--cookies",
            f"{self.config.resources_folder}/cookies.txt",
            "--check-formats",
            "--rm-cache-dir",
            "-4",
            "--audio-format",
    ...
    async def _play_loop(self, guild_id: int, user_triggered: bool = False):
        while (sound_queue := self.sound_queue.get(guild_id, [])) != []:
            ...
            if sound_type == "file":
                ...
                await self.refresh_youtube_cookies()
                success, path_or_error = await self._download_from_url(sound_name)

                if success:
                    sound_path = path_or_error
                    title = os.path.basename(sound_path).split(".")[0].replace("_", " ")
                    title = f'"{title}"'
                    volume = 0.2
                else:
                    await message.channel.send(path_or_error)
    ...
    def _generate_cookies_file(self, cookies: Dict[str, Any]):
        with open(f"{self.config.resources_folder}/cookies.txt", "w", encoding="utf-8") as fp:
            fp.write("# Netscape HTTP Cookie File\n")
            fp.write("# This file is generated by yt-dlp.  Do not edit.\n\n")

            for cookie_data in cookies:
                domain = cookie_data["domain"]
                subdomains = "TRUE" if (cookie_data["sameSite"] != "Strict") else "FALSE"
                path = cookie_data["path"]
                secure = "TRUE" if cookie_data["httpOnly"] else "FALSE"
                expires = int(cookie_data["expires"])
                if expires == -1:
                    continue
                ...
                fp.write(f"{domain}\t{subdomains}\t{path}\t{secure}\t{expires}\t{name}\t{value}\n")

    async def refresh_youtube_cookies(self):
        async with async_playwright() as playwright:
            ...
            cookie_dialog = await page.wait_for_selector("#dialog")

            spans = await cookie_dialog.query_selector_all("span[role=text]")
            for span in spans:
                if (await span.text_content()).strip().lower() == "reject all":
                    await span.click()

            await asyncio.sleep(1)

            cookies = await page.context.cookies("https://youtube.com")
            self._generate_cookies_file(cookies)
